...
def make_new_pass():
    ...
    with open(home + '\\python_key_netezza.key', 'wb') as k:
        k.write(key)
    with open('H:\\python_key_netezza.creds', 'wb') as k:
        k.write(cipher_text)


def get_pass():
    """ Read the key and cipher text and return the decoded password"""
    with open(home + '\\python_key_netezza.key', 'rb') as k:
        key = k.read()
    with open('H:\\python_key_netezza.creds', 'rb') as k:
        cipher_text = k.read()
